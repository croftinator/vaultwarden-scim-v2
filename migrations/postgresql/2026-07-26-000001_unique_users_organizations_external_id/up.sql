...
-- migration, and dropped the first in a third - three statements and two rounds
-- of index building for one index. Collapsed while this branch is still
...
-- `akey` and no access - only a correlation hint the next sync re-establishes.
...
-- time component. It is NOT "keep the oldest", and there is no better option
-- here - `users_organizations` carries no creation timestamp.
--
-- The length clause is the postgresql-only hazard. `external_id` is TEXT here
-- with no cap, and the one writer that predates SCIM - the Directory Connector
-- import - applies no length check at all, so a deployment using LDAP DNs as
-- externalIds can hold arbitrarily long values. A plain btree index entry caps at
-- about 2704 bytes, so a single over-long pre-existing row would make the CREATE
-- below fail outright and leave a server that will not start. SCIM caps
-- externalId at 300 characters (SCIM_MAX_EXTERNAL_ID_LEN), so anything longer was
-- never SCIM's to begin with.
--
