...
-- and for why this is one migration rather than the three it used to be.
--
...
-- resolves the conflict by DELETING the conflicting row, taking its
-- `collections_groups` access grants with it. SCIM writes go through
-- `save_strict`, never `save`. See `Group::save_strict`.
--
-- `groups.external_id` is VARCHAR(300) here, matching SCIM_MAX_EXTERNAL_ID_LEN,
-- so this needs no length guard - unlike `users_organizations.external_id`,
-- which is unbounded TEXT on postgresql.
...
-- users_organizations one, which has no such column, so the two read the same way
-- and neither implies a guarantee the other cannot make.
--
